...
import { expensePerCategoryStyles } from "./ExpensePerCategory.style";
import { PieChart } from "react-native-gifted-charts";
import { useCallback, useEffect, useState } from "react";
import {
  calculateExpensesPerCategory,
  ExpenseByCategory,
} from "@/utils/calculateExpensesPerCategory";
...
export const ExpensePerCategory = ({ convertedRecords, showTitle = true, animationDelay = 450, isReportsLoading = false }: ExpensePerCategoryProps) => {
  const { theme } = useTheme();
  const styles = expensePerCategoryStyles(theme);
  const [data, setData] = useState<ExpenseByCategory[]>([]);
  const { user } = useSession();
  const { records } = useRecords();
  const { t } = useTranslation();
  const { isLoading, selectedReportCategories } = useFilter();

  const fetchData = async () => {
    const data = await calculateExpensesPerCategory(
      user?.mainCurrency || "usd",
      convertedRecords || records || [],
      selectedReportCategories
    );
    setData(data as ExpenseByCategory[]);
  };
  ...
  return (
    <Animated.View style={[styles.container, displayAnimation]}>
      {showTitle && <Text variant="body20_bold">{t('expensePerCategory.title')}</Text>}
        ...
        <View style={styles.contentContainer}>
          {data.length > 0 ? data.map((item, index) => (
            ...
            <View style={[styles.itemContainer, { opacity: 0.3, justifyContent: "center", alignItems: "center" }]}>
              <Text variant="body14_medium" style={{ textAlign: "center", color: theme.colors.text.primary, maxWidth: 140 }}>{t('expensePerCategory.noExpenses')}</Text>
            </View>
          )}
        </View>
